...
import { isMobile } from 'app/scripts/helpers';
      ...
      <div className={`feedcast feedcast--sidebar-${this.state.showSidebar?'active':'inactive'}`}>
        ...
        <div className={`feedcast__sidebar feedcast__sidebar--${this.state.showSidebar ? 'show':'hide'}`}>
          <div className="feedcast__sidebar-wrapper">
            <NavLink exact onClick={()=>{this.toggleSidebar(false)}} activeClassName="active" to="/"><i className="fa fa-home"></i> {t('home.title')}</NavLink>
            <NavLink onClick={()=>{this.toggleSidebar(false)}} activeClassName="active" to="/episodes"><i className="fa fa-history"></i> {t('episodes.title')}</NavLink>
            <NavLink onClick={()=>{this.toggleSidebar(false)}} activeClassName="active" to="/channels"><i className="fa fa-rss"></i> {t('channels.title')}</NavLink>
            <NavLink onClick={()=>{this.toggleSidebar(false)}} activeClassName="active" to="/queue"><i className="fa fa-indent"></i> {t('queue.title')}</NavLink>
